...
@section('scripts')
    <script>
        ...
        window.taxPercent = '{{ tax_percent(currency_symbol()) }}';
...
@section('content')
    ...
    <div class="max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto">
        <div class="grid grid-cols-3 gap-4">
            ...
            @else
                <div class="col-span-3 md:col-span-2">
        <form method="POST" action="{{ route('free_trial.config', ['product' => $product]) }}{{(request()->getQueryString() != null ? '?' . request()->getQueryString() : '')}}" id="free_trial_form">
            <input type="hidden" name="currency" value="{{ currency_symbol() }}" id="currency">
            @csrf
            @include("shared.alerts")
            <h1 class="text-2xl font-semibold mb-4 dark:text-white">{{ __('free_trial::lang.config.title') }}</h1>
                    @php($pricings = $product->pricingAvailable(currency()))
            ...
            @endif
                <div class="col-span-3 md:col-span-1">
                    <div class="card dark:text-gray-400">
                        ...
                        <button class="bg-indigo-600 text-white py-2 px-4 rounded-lg mt-4 w-full" onclick="document.querySelector('#free_trial_form').submit()">{{ __('free_trial::lang.config.start') }}</button>
